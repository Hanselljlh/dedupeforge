...
- [x] Add error test for unreadable files (Unix; Windows `chmod` is not reliable for this).
- [x] Decide whether zero-byte files should be included by default — excluded from exact
      scan (`min_size >= 1`); use `EmptyFiles` mode to surface them explicitly.
