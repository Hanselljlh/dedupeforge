...
#[test]
fn duplicate_archive_member_mode_reports_archive_duplicates() {
    use std::io::Write;

    let root = std::env::temp_dir().join("dedupeforge-cli-archive-members");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let left_zip = root.join("left.zip");
    let right_zip = root.join("right.zip");

    let file = std::fs::File::create(&left_zip).unwrap();
    ...
    let file = std::fs::File::create(&right_zip).unwrap();
    let mut zip = zip::ZipWriter::new(file);
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_dedupeforge"))
        .arg(root.as_os_str())
        .arg("--mode")
        .arg("duplicate-archive-members")
        .output()
        .unwrap();

    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("Mode: duplicate-archive-members"));
    assert!(stdout.contains("Match risk: medium"));
    assert!(stdout.contains("same archive member size + same full hash"));

    let _ = std::fs::remove_dir_all(&root);
}

#[test]
fn empty_archive_mode_reports_empty_zip_files() {
    let root = std::env::temp_dir().join("dedupeforge-cli-empty-archives");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let empty_zip = root.join("empty.zip");
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("Mode: empty-archives"));
    assert!(stdout.contains("Match risk: low"));
    assert!(stdout.contains("archives with no file members"));
